...
func CalculateTimeInterval(articles []*TopArticle, session *mgo.Session) {
	if session == nil {
		Debugger.Printf("No DB session, cannot calculate time intervals")
		return
	}
...
*/
func SaveTopPagesSnapshot(toppages []*TopArticle, session *mgo.Session) error {
